...
import { ActivityIndicator, Dimensions, FlatList, Image, SafeAreaView, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch('http://172.16.59.43:3000/api/barang') // Ganti dengan IP lokal kamu
      .then((res) => res.json())
      .then((data) => {
        setProdukList(data.barang || []);
        setLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      {loading ? (
        <ActivityIndicator size="large" color="#26C2FF" style={{ marginTop: 50 }} />
      ) : (
        <FlatList
          ...
          ListHeaderComponent={
            <>
              ...
              >
                <Text style={{ color: '#f2f2f2', fontSize: 14, marginBottom: 5, fontFamily: 'Poppins' }}>
                  Selamat Pagi,
                </Text>
                <Text style={{ color: '#fff', fontSize: 30, fontWeight: 'bold', marginBottom: 10, fontFamily: 'Montage' }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 24, paddingHorizontal: 10 }}>
                {[
                  { title: 'HARGA YANG RASIONAL', desc: 'Barang berkualitas, harga bersahabat.', icon: 'pricetag-outline' },
                  { title: 'BARANG NGGAK ASAL', desc: 'Asli, bebas cacat, dan layak pakai.', icon: 'checkmark-done-outline' },
                  { title: 'KEUNTUNGAN MAKSIMAL', desc: 'Tukar poin jadi diskon atau hadiah.', icon: 'gift-outline' },
                ].map((item, index) => (
                  <LinearGradient
                    key={index}
                    colors={['#220593', '#26C2FF']}
                    ...
                    style={{ flex: 1, marginRight: index < 2 ? 8 : 0, borderRadius: 20, padding: 12 }}
                  >
          ...
          renderItem={({ item }) => (
            <View style={{ width: itemWidth, backgroundColor: '#f7f7f7', borderRadius: 15, padding: 10, position: 'relative' }}>
              ...
              <Text style={{ fontSize: 12, fontWeight: '600', marginBottom: 4 }}>{item.nama_barang}</Text>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ fontSize: 12, color: '#888' }}>Rp{item.harga_barang}</Text>
